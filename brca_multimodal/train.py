...
parser.add_argument('--seed', '-s', type=int, default=36,
                        help='Random seed')
...
parser.add_argument('--gene_expr', type=str, default='/work/ai4bio2024/brca_surv/LAB_WSI_Genomics/TCGA_BRCA/Data/genomic/tpm_unstranded.csv',
                        help='Gene exprection file')
...
gene_exp = pd.read_csv(args.gene_expr, sep='\t')

gene_exp = gene_exp.drop(columns=['ENSG00000012048.23', 'ENSG00000139618.16'])
gene_exp = gene_exp.rename(columns = {'Unnamed: 0':'case_id'})
...
for f, (train_index, val_index) in enumerate(skf.split(X, y)):
    ...
    train_dataset = UNIDataset(gene_expr=gene_exp, data_frame=train_val_df.iloc[train_index], data_dir=data_dir, label = args.label, seed=SEED, max_patches=4096)
    val_dataset = UNIDataset(gene_expr=gene_exp, data_frame=train_val_df.iloc[val_index], data_dir=data_dir, label = args.label, seed=SEED, max_patches=4096)
    ...
    for e in range(EPOCHS):
        ...
        for idx, (data, genes, label) in enumerate(tqdm(train_loader)):
            data = data.to(device)
        ...
        val_loss = 0.0
        with torch.no_grad():
            for data, genes, label in tqdm(val_loader):
                data = data.to(device)
                genes = genes.to(device)
                label = label.to(device)
                output, _ = model(data, genes)
