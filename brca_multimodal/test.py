...
test_dataset = UNIDataset(gene_expr=gene_exp, data_frame=test_df, data_dir=data_dir, label = args.label, seed=SEED, max_patches=4096)
...
for f in range(FOLDS):
    ...
    with torch.no_grad():
        y_pred = []
        y_true = []
        for data, genes, label in tqdm(test_loader):
            data = data.to(device)
