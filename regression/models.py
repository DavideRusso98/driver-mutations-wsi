...
class BMClassifier(nn.Module):
    def __init__(self, input_size = 1024, output_dim = 1, inner_dim = 128, dropout_v=0.0, nonlinear=True, passing_v=True): # K, L, N
        super(BMClassifier, self).__init__()
        if nonlinear:
            self.q = nn.Sequential(nn.Linear(input_size, inner_dim), nn.ReLU(), nn.Linear(inner_dim, inner_dim), nn.Tanh())
        else:
            self.q = nn.Linear(input_size, inner_dim)
    ...
    def forward(self, feats, c):
        ...
        reduced_features = feats[0, top_idx, :].unsqueeze(0)

        V = self.v(reduced_features)
        Q = self.q(reduced_features)
        ...
        att = torch.zeros((1, 1, feats.shape[1]), dtype=torch.float, device=device)
        att[0, 0, top_idx] = A.squeeze()
